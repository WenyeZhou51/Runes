...
namespace BulletHell
{
    ...
    public class ProjectileEmitterAdvanced : ProjectileEmitterBase
    {
        ...
        [ConditionalField(nameof(UseFollowTarget))] private Transform Target;
        ...
        public new void Awake()
        {
            ...
            contactFilter.SetLayerMask(wallLayer | playerLayer);
        ...
        private Vector2 RotateByRandomAngle(Vector2 normalVector, float angle)
        {
            // Ensure the input vector is normalized
            normalVector.Normalize();

            // Generate a random angle in degrees within the range [-angle, angle]
            float randomAngle = Random.Range(-angle, angle);

            // Convert the random angle to radians
            float randomAngleRad = randomAngle * Mathf.Deg2Rad;

            // Calculate the cosine and sine of the random angle
            float cos = Mathf.Cos(randomAngleRad);
            float sin = Mathf.Sin(randomAngleRad);

            // Rotate the vector using the rotation matrix
            float newX = normalVector.x * cos - normalVector.y * sin;
            float newY = normalVector.x * sin + normalVector.y * cos;
            ...
            for (int g = 0; g < GroupCount; g++)
            {
                if (Projectiles.AvailableCount >= SpokeCount)
                {
                    ...
                    for (int n = 0; n < SpokeCount; n++)
                    {
                        ...
                        {
                            Groups[g].Direction =  RotateByRandomAngle((Target.transform.position - transform.position).normalized,deviation)  ;
            ...
            if (node.Active)
            {
                ...
                if (node.Item.TimeToLive > 0)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        Collider2D hitCollider = RaycastHitBuffer[0].collider;
                        if (hitCollider != null)
                        {
                            //collide with non-collidable
                            if (((1 << hitCollider.gameObject.layer) & wallLayer) == 0 && ((1 << hitCollider.gameObject.layer) & playerLayer) == 0)
                            {
                                // The object is not in the specified layers, so ignore this collision
                            ...
                            //colide with wall
                            else if (((1 << hitCollider.gameObject.layer) & wallLayer) != 0)
                            {
                            ...
                            //colide with player
                                ...
                                if (!player.invincible)
                                {
                                    if (player != null)
                                    {
                                        int damage = ProjectilePrefab.damage;
